...
def affiliation(session: Session) -> None:
    ...
    _corporation = AffiliationTable(name=company_structure["name"])  # type: ignore
    session.add(_corporation)
    session.commit()

    def recursion(children: list, node_id: int) -> None:
        for item in children:
            _item = AffiliationTable(name=item.get("name"), nodeId=node_id)
            session.add(_item)
            session.commit()

            if item.get("children"):
                recursion(item["children"], _item.id)  # type: ignore

    recursion(company_structure["children"], _corporation.id is not None)  # type: ignore
